  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <StatTile icon={ListTodo} label="Total Activities" value={total} />
        <StatTile icon={CheckCircle2} label="Completed" value={completed} accent="text-emerald-600" />
        <StatTile icon={Clock} label="In Progress" value={inProgress} accent="text-blue-600" />
        <StatTile icon={CalendarClock} label="Upcoming" value={upcoming} accent="text-amber-600" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-card border border-slate-200 shadow-card p-5">
          <h2 className="font-semibold text-slate-900 mb-3">Activity Timeline</h2>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-card border border-slate-200 shadow-card p-5">
            <h3 className="font-semibold text-slate-900 mb-2 flex items-center gap-2"><Users className="h-4 w-4" /> Personnel Involved</h3>
            {uniquePersonnel.length === 0 ? <p className="text-sm text-slate-400">None yet.</p> : (
              <ul className="text-sm text-slate-600 space-y-1">{uniquePersonnel.slice(0, 10).map(p => <li key={p.id}>{p.name}</li>)}</ul>
            )}
          </div>

          <div className="bg-white rounded-card border border-slate-200 shadow-card p-5">
            <h3 className="font-semibold text-slate-900 mb-2 flex items-center gap-2"><Navigation className="h-4 w-4" /> Latest GPS Execution</h3>
            {latestGpsActivity ? (
              <div className="text-sm text-slate-600">
                <p>{latestGpsActivity.title}</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  {latestGpsActivity.gps_validation_status === 'valid' ? 'Valid' : latestGpsActivity.gps_validation_status?.replace('_', ' ')}
            ...
            ) : <p className="text-sm text-slate-400">No GPS captures yet.</p>}
          </div>

          <div className="bg-white rounded-card border border-slate-200 shadow-card p-5">
            <h3 className="font-semibold text-slate-900 mb-2 flex items-center gap-2"><Camera className="h-4 w-4" /> Recent Evidence</h3>
            {thumbsError && (
              <div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-control px-3 py-1.5 mb-2">
                ...
                <span className="flex-1">Could not load previews.</span>
                <button onClick={retryThumbs} className="font-medium underline">Retry</button>
              </div>
            )}
            {recentEvidence.length === 0 ? <p className="text-sm text-slate-400">No photos yet.</p> : (
          ...
          {latestActivity && (
            <div className="bg-white rounded-card border border-slate-200 shadow-card p-5">
              <h3 className="font-semibold text-slate-900 mb-2">Latest Activity</h3>
